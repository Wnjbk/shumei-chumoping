...
static void ilitek_poll_work_fn(struct work_struct *work)
{
	if (ilits && ilits->boot && ilits->input && ilits->report &&
		...
		ili_report_handler_poll();
		mutex_unlock(&ilits->touch_mutex);
	}

	if (READ_ONCE(ilitek_polling))
		schedule_delayed_work(&ilitek_poll_work,
				      msecs_to_jiffies(ILITEK_POLL_MS));
}
...
{
	WRITE_ONCE(ilitek_polling, false);
	cancel_delayed_work_sync(&ilitek_poll_work);
}

int ili_irq_register(int type)
{
	INIT_DELAYED_WORK(&ilitek_poll_work, ilitek_poll_work_fn);
	atomic_set(&ilits->irq_stat, ENABLE);
	if (READ_ONCE(ilitek_polling)) {
		schedule_delayed_work(&ilitek_poll_work,
				      msecs_to_jiffies(ILITEK_POLL_MS));
		ILI_INFO("P03 polling enabled at %d ms\n", ILITEK_POLL_MS);
	} else {
		ILI_INFO("P03 polling disabled; load with polling=1 to enable\n");
	}
	return 0;
}
...
#else
#if 0 /* Raspberry Pi KMS has no legacy framebuffer notifier API. */
...
{
#if 0 /* Display suspend integration is added through DRM separately. */
