...
static int ili_report_handler_internal(bool polling)
{
	int ret = 0, pid = 0;
	...
	if (polling) {
		ret = ili_i2c_poll_read(ilits->tr_buf, rlen);
		if (ret != rlen)
			goto out; /* No queued report; leave the display/firmware alone. */
	} else {
...
{
	return ili_report_handler_internal(false);
}

int ili_report_handler_poll(void)
{
	return ili_report_handler_internal(true);
}
