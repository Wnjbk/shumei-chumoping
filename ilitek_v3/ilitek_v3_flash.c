...
static int ilitek_tddi_flash_poll_busy(int timer)
{
	...
	if (ilits->isSupportFlash)
		cmd = flash_protect_list[ilits->supportFlashIndex].readOperator[0];
...
static int ilitek_tddi_fw_flash_erase(bool mcu)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		for (addr = (fbi[i].start & (~0xFFF)); addr <= fbi[i].end; addr += ilits->flash_sector) {
			ilitek_tddi_flash_write_enable();
	...
	for (i = 0; i < FW_BLOCK_INFO_B1_NUM; i++) {
		...
		for (addr = (fbi_b1[i].start & (~0xFFF)); addr <= fbi_b1[i].end; addr += ilits->flash_sector) {
			ilitek_tddi_flash_write_enable();
...
static void ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	fbi[AP_OVERLAY].name = "AP_OVERLAY";

	/* upgrade mode define */
	fbi[DATA].mode = fbi[AP].mode = fbi[TUNING].mode = AP;
	fbi[AP_OVERLAY].mode = AP;
	fbi[MP].mode = MP;
...
{
	int i = 0, size, blk_num = 0, blk_map = 0, num;
	int b0_addr = 0, b0_num = 0;
	/*b2 tag*/
	int ili_ver = 0, af_block_num = 0, ili_file_header_len = 0;
	int offset = 0;
	bool ili_format_ver_00 = false;
	...
	if ((blk_num & BIT(7)) != BIT(7) && (blk_num & BIT(6)) != BIT(6)) {
		ili_format_ver_00 = true;
		ili_file_header_len = ILI_FILE_HEADER;
	} else if ((blk_num & BIT(7)) != BIT(7) && (blk_num & BIT(6)) == BIT(6)) {
		ili_file_header_len = (CTPM_FW[129] << 8) | CTPM_FW[130];
	} else {
		ILI_ERR("Invaild ILI format version, abort!\n");
		return -EINVAL;
	}
	...
	if (ili_format_ver_00) {
		...
		while (i < blk_num) {
			...
			af_block_num = CTPM_FW[6 + i * 7 + offset];
			if (af_block_num >= FW_BLOCK_INFO_NUM) {
				ILI_ERR("AF Tag index is out of range of defined block array, abort!\n");
				return -EINVAL;
			}
			fbi[af_block_num].start = (CTPM_FW[0 + i * 7 + offset] << 16) | (CTPM_FW[1 + i * 7 + offset] << 8) | CTPM_FW[2 + i * 7 + offset];
			fbi[af_block_num].end = (CTPM_FW[3 + i * 7 + offset] << 16) | (CTPM_FW[4 + i * 7 + offset] << 8) | CTPM_FW[5 + i * 7 + offset];
