...
int ili_move_mp_code_flash(void)
{
	...
	if (overlay_drv_enable == 1) {
		...
		if (ilits->cascade_info_block.nNum != 0) {
			/* stop clock */
			if (ili_ice_mode_bit_mask_write_cascade(FLASH_DMA_CHANNEL_SEL_ADDR, FLASH_reg_stop_clk_en, BIT(24), BOTH) < 0)
				ILI_ERR("Cascade mask write: mask=%lu, value=%lu, addr=0x%x failed\n", FLASH_reg_stop_clk_en, BIT(24), FLASH_DMA_CHANNEL_SEL_ADDR);
			/* set dma channel 0 */
			if (ili_ice_mode_bit_mask_write_cascade(FLASH_DMA_CHANNEL_SEL_ADDR, FLASH_reg_dma_channel_sel, 0, BOTH) < 0)
				ILI_ERR("Cascade mask write: mask=%lu, value=%d, addr=0x%x failed\n", FLASH_reg_dma_channel_sel, 0, FLASH_DMA_CHANNEL_SEL_ADDR);
			...
			/* stop clock */
			if (ili_ice_mode_bit_mask_write(FLASH_DMA_CHANNEL_SEL_ADDR, FLASH_reg_stop_clk_en, BIT(24)) < 0)
				ILI_ERR("Mask write: mask=%lu, value=%lu, addr=0x%x failed\n", FLASH_reg_stop_clk_en, BIT(24), FLASH_DMA_CHANNEL_SEL_ADDR);
			/* set dma channel 0 */
			if (ili_ice_mode_bit_mask_write(FLASH_DMA_CHANNEL_SEL_ADDR, FLASH_reg_dma_channel_sel, 0) < 0)
				ILI_ERR("Mask write: mask=%lu, value=%d, addr=0x%x failed\n", FLASH_reg_dma_channel_sel, 0, FLASH_DMA_CHANNEL_SEL_ADDR);
