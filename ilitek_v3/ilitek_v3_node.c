...
int ratio_percent(int num, int den)
{
	...
	return sign * (int)div64_s64(n * 100, d);
}
...
static int ilitek_proc_debug_all_message_show(struct seq_file *m, void *v)
{
	...
	do {
		if (ilits->dbf_out[count].mark) {
			...
			if (need_read_data_len > TR_BUF_SIZE) {
				ILI_INFO("need_read_data_len(0x%x) is error.\n", need_read_data_len);
			} else {
...
static int ilitek_proc_debug_message_show(struct seq_file *m, void *v)
{
	...
	if (ilits->dbl[ilits->odi].mark) {
		...
		if (need_read_data_len < TR_BUF_SIZE) {
			...
			memset(ilits->outputStrArr, 0, need_read_data_len * 2 + 2);
