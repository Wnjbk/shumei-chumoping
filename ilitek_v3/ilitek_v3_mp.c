...
static struct core_mp_test_data {
	...
	bool td_retry;
	bool pre_cmd_sp;
	bool is_openc_test;/*flag that is "open C" been tested*/
	...
	int tipring_len;
	int openc_idx;/*open c item index of test item array*/
...
static struct mp_test_items tItems[MP_TEST_ITEM] = {
	...
	{.desp = "open test_cc2", .catalog = OPEN_TEST, .lcm = ON},
...
static void mp_print_csv_cdc_cmd(char *csv, int *csv_len, int index, int file_size)
{
	...
	} else if ((ipio_strcmp(name, "open test_c") == 0) || (ipio_strcmp(name, "open test_cc2") == 0)) {
...
static int open_test_cc2(int index)
{
	int i = 0, x = 0, y = 0, ret = 0, each_data_idx = 0, each_frame_idx = 0;
	const s32 *open_c_value;/*point to open c value buffer*/
	s32 *caculate_value;/*point to open cc value buffer*/
	int cal_L_R = 0, cal_R_L = 0, left_divide = 0, right_divide = 0;

	if (core_mp.openc_idx < 0) {
		ret = -1;
		ILI_ERR("The “open test_cc2” can only be tested when “open test_c” is enabled, please check the configuration\n");
		goto out;
	}

	if (!core_mp.is_openc_test) {
		ret = -1;
		ILI_ERR("The “open test_c” must be performed before the “open test_cc2”\n");
		goto out;
	}
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		each_frame_idx = i * core_mp.frame_len;
		open_c_value = &tItems[core_mp.openc_idx].buf[each_frame_idx];
		caculate_value = &tItems[index].buf[each_frame_idx];
		each_data_idx = 0;
		...
		for (y = 0; y < core_mp.ych_len; y++) {
			for (x = 0; x < core_mp.xch_len; x++) {
				/*left to right*/
				if (x == core_mp.xch_len - 1) {
					left_divide = open_c_value[each_data_idx];
					right_divide = open_c_value[each_data_idx - x];/*take the start element of this row*/
				} else {
					left_divide = open_c_value[each_data_idx];
					right_divide = open_c_value[each_data_idx + 1];
				}
				...
				cal_L_R = ratio_percent(left_divide, right_divide);
				
				/*right to left*/
				if(x == 0) {
					left_divide = open_c_value[each_data_idx + core_mp.xch_len - 1];/*take the end element of this row*/
					right_divide = open_c_value[each_data_idx];
				} else {
					left_divide = open_c_value[each_data_idx - 1];
					right_divide = open_c_value[each_data_idx];
				}
				if (left_divide == 0) {
					left_divide = 1;
				...
				cal_R_L = ratio_percent(right_divide, left_divide);

				/*assign value*/
				caculate_value[each_data_idx] = min(cal_L_R, cal_R_L);
				each_data_idx++;
			}
		}
		
		compare_MaxMin_result(index, &tItems[index].buf[each_frame_idx]);
	}
...
static int mp_show_result(bool lcm_on)
{
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		if ((ipio_strcmp(tItems[i].desp, "open test_c") == 0) || (ipio_strcmp(tItems[i].desp, "open test_cc2") == 0)) {
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		...
		if (tItems[i].catalog == MUTUAL_TEST) {
			...
			if (ipio_strcmp(tItems[i].desp, "open test(integration)_sp") == 0) {
				...
				core_mp.openc_idx = i;
			} else if (ipio_strcmp(tItems[i].desp, "open test_cc2") == 0) {
				tItems[i].do_test = open_test_cc2;
