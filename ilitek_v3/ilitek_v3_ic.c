...
/* Page 6 settings supplied for the HNH07 720x1280 two-lane panel. */
int ili_tddi_panel_set_2lane(void)
{
	static const struct {
		u8 reg;
		u8 expected;
	} panel_regs[] = {
		{ 0xd9, 0x0f }, /* two DSI lanes */
		{ 0xc0, 0x00 }, { 0xc1, 0x15 }, /* 1280 lines */
		{ 0xc3, 0x0e }, /* scan direction and RGB order */
		{ 0x0d, 0x8a }, { 0x0e, 0x67 }, /* 122 MHz PLL */
		{ 0xd6, 0x55 }, { 0xdc, 0x88 },
	};
	u8 value = 0xff;
	int attempt, i, ret = 0;

	if (!ilits || !ilits->i2c || !ilits->chip || !ilits->wrapper)
		return -ENODEV;
	...
	for (i = 0; i < ARRAY_SIZE(panel_regs); i++) {
		for (attempt = 0; attempt < 3; attempt++) {
			value = 0xff;
			ili_ic_get_ddi_reg_onepage(0x06, panel_regs[i].reg,
						&value, false, false);
			if (value == panel_regs[i].expected)
				break;
			ili_ic_set_ddi_reg_onepage(0x06, panel_regs[i].reg,
						 panel_regs[i].expected, false, false);
		...
		ili_ic_get_ddi_reg_onepage(0x06, panel_regs[i].reg,
						&value, false, false);
		if (value != panel_regs[i].expected) {
			ILI_ERR("DDI page 6 reg 0x%02x: expected 0x%02x, got 0x%02x\n",
				panel_regs[i].reg, panel_regs[i].expected, value);
			ret = -EIO;
			break;
		}
	}
	...
	if (!ret)
		ILI_INFO("HNH07 page 6 panel settings verified (2 lanes, 1280 lines)\n");
	return ret;
}
EXPORT_SYMBOL_GPL(ili_tddi_panel_set_2lane);

/* P5_X_SET_DDI_STATUS (0xd1) is forwarded by the vendor wrapper using the
 * Linux i2c_transfer() API. The page-selection packet is represented by the
 * page argument to 0xd1, rather than being sent as a separate DSI packet.
 * For Gamma, send the entire 27-byte payload in one I2C transfer. This
 * extension to the single-byte vendor helper still needs panel readback.
 */
int ili79505a_panel_run_test_sequence(void)
{
	u8 page = 0, tx[3 + 27];
	u8 lane = 0xff, tp_mode = 0xff;
	unsigned int i;
	int ret;

	if (!ilits || !ilits->i2c || !ilits->wrapper)
		return -ENODEV;

	ret = ili_tddi_panel_set_2lane();
	if (ret)
		return ret;

	mutex_lock(&ilits->touch_mutex);
	for (i = 0; i < ARRAY_SIZE(ili79505a_test_init); i++) {
		const struct ili79505a_test_step *step = &ili79505a_test_init[i];
		u8 len;

		if (step->delay_ms) {
			msleep(step->delay_ms);
			continue;
		}
		len = step->packet_len;
		if (len == 4 && step->packet[0] == 0xff &&
		    step->packet[1] == 0x5a && step->packet[2] == 0xa5) {
			page = step->packet[3];
			continue;
		}
		if (!len || len > 28) {
		...
		tx[0] = P5_X_SET_DDI_STATUS;
		tx[1] = page;
		tx[2] = step->packet[0];
		memcpy(&tx[3], &step->packet[1], len - 1);
		ret = ilits->wrapper(tx, len + 2, NULL, 0, OFF, OFF);
		if (ret < 0) {
			ILI_ERR("ILI79505A TEST step %u page %02x reg %02x I2C error %d\n",
				i, page, step->packet[0], ret);
			goto out;
		}
	}
	/* Check two distinctive TEST values through the vendor DDI read path. */
	ret = ili_ic_get_ddi_reg_by_fw(0x06, 0xd9, &lane);
	if (ret < 0 || lane != 0x0f) {
		ret = -EIO;
		goto out;
	}
	ret = ili_ic_get_ddi_reg_by_fw(0x0e, 0x00, &tp_mode);
	if (ret < 0 || tp_mode != 0xa0) {
		ret = -EIO;
		goto out;
	}
	ret = 0;
	ILI_INFO("ILI79505A TEST sequence transferred over I2C: %zu steps\n",
		 ARRAY_SIZE(ili79505a_test_init));
out:
	mutex_unlock(&ilits->touch_mutex);
	return ret;
}
EXPORT_SYMBOL_GPL(ili79505a_panel_run_test_sequence);
...
void ili_ic_get_pc_counter(int stat)
{
	...
	u32 crash = 0, crash_addr = 0x40054;
	int ret = 0;
#if (TDDI_INTERFACE == BUS_SPI)
	u32 mcu_state = 0;
#endif
	...
	if (ilits->cascade_info_block.nNum != 0) {
		...
		ili_cascade_rw_tp_reg(mcu_state, ILI_READ, crash_addr, 0, 0, &crash, true);

		ilits->fw_pc = pc;
		ilits->fw_latch = latch;

		if (ilits->slave_wr_ctrl) {
			ILI_ERR("Read Slave counter (addr: 0x%x) = 0x%x, latch (addr: 0x%x) = 0x%x, crash (addr: 0x%x) = 0x%x\n",
				pc_addr, ilits->fw_pc, latch_addr, ilits->fw_latch, crash_addr, crash);
		} else {
			ILI_ERR("Read Master counter (addr: 0x%x) = 0x%x, latch (addr: 0x%x) = 0x%x, crash (addr: 0x%x) = 0x%x\n",
				pc_addr, ilits->fw_pc, latch_addr, ilits->fw_latch, crash_addr, crash);
		}
		...
		ili_ice_mode_read_by_mode(crash_addr, &crash, sizeof(u32), ilits->slave_wr_ctrl);
		...
		if (ili_ice_mode_read(crash_addr, &crash, sizeof(u32)) < 0)
			ILI_ERR("Read pc crash error\n");

		ilits->fw_pc = pc;
		ilits->fw_latch = latch;
		ILI_ERR("Read counter (addr: 0x%x) = 0x%x, latch (addr: 0x%x) = 0x%x, crash (addr: 0x%x) = 0x%x\n",
			pc_addr, ilits->fw_pc, latch_addr, ilits->fw_latch, crash_addr, crash);
...
out:
	...
	if (ilits->chip->core_ver >= CORE_VER_1700) {
		...
		if (ilits->cascade_info_block.nNum != CASCADE_NUM) {
			ILI_INFO("**Cascade number mismatch, detected : %d, configured : %d. Please check CASCADE_NUM setting.**\n", ilits->cascade_info_block.nNum, CASCADE_NUM);
		}
	}
