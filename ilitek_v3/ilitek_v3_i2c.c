...
/* Polling has no IRQ/status gate: a missing report is an expected idle read. */
int ili_i2c_poll_read(u8 *buf, int len)
{
	if (!ilits || !ilits->i2c || !buf || len <= 0 || len > TR_BUF_SIZE)
		return -EINVAL;
	return core_i2c_read(buf, len);
}
...
#endif
{
	...
	/* Raspberry Pi uses the vendor Intel HEX image from /lib/firmware. */
